...
class TestResidualZeroCorpus(unittest.TestCase):
    ...
    def test_no_residual_sets_across_corpus_sample(self):
        """xfail (operator-authorized): the residual-drain work this gate guards is in progress, so
        residual SETs are expected non-zero until it lands. Remove this decorator when the drain
        completes -- an unexpected pass then flags that it is done."""
        allf = sorted(
