...
def test_explosion_raises_when_residual_payloads_exceed_input():
    """One shape but 8 distinct residual payloads -> minted vocabulary exceeds input."""
    reg = _two_note_input()
...
def test_no_explosion_when_shapes_reuse_one_gesture():
    """One shape (shared cb), residuals all identical -> 2 keys <= 2 input notes."""
    reg = _two_note_input()
